 ...
 * melee-punish phases, and the car (dash) phase danger path plus the safe tiles
 * shielded by arena boulders.
 */
public class MokhaiotlOverlay extends Overlay {
    ...
    private static final Color DASH_PATH_COLOR = new Color(238, 174, 125);
    private static final Color CAR_SAFE_TILE_COLOR = new Color(145, 215, 210);
    private static final Color MELEE_PUNISH_TILE_COLOR = new Color(225, 150, 165);
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if (config.showMokhaiotlCarSafeTiles()) {
            renderCarSafeTiles(graphics, handler);
        }
    ...
    private void renderStatues(Graphics2D graphics, MokhaiotlHandler handler) {
        ...
        for (NPC npc : handler.getHighlightedStatues()) {
    ...
    private void renderDashPath(Graphics2D graphics, MokhaiotlHandler handler) {
        ...
        Color fill = withAlpha(base, config.mokhaiotlTransparency());
        Color border = withAlpha(base, Math.min(255, config.mokhaiotlTransparency() + 100));
        ...
        // Stroke only the outer perimeter of the path shape.
        graphics.setColor(border);
        graphics.setStroke(PATH_BORDER_STROKE);
        for (WorldPoint tile : path) {
            Polygon poly = worldTilePoly(tile);
            if (poly == null || poly.npoints < 4) {
                continue;
            }
            if (!path.contains(tile.dy(-1))) {
                drawEdge(graphics, poly, 0, 1);
            }
            if (!path.contains(tile.dx(1))) {
    ...
    private void renderCarSafeTiles(Graphics2D graphics, MokhaiotlHandler handler) {
        Set<WorldPoint> tiles = handler.getCarSafeTiles();
        if (tiles.isEmpty()) {
            return;
        }
        Color base = CAR_SAFE_TILE_COLOR;
        for (WorldPoint tile : tiles) {
            renderWorldTile(graphics, tile, base, BORDER_STROKE);
        }
    }
